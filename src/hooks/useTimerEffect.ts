...
const useTimerEffect = ({
    ...
    completeReset,
    SciFiAlarm
} : { isRunning: boolean; worker: Worker | null; count: number; isBreak: boolean; breakDuration: number; isAlertOn: boolean; completeReset: () => void; SciFiAlarm: string }) => {
    ...
    useEffect(() => {
        ...
        const shouldAlert = (isBreak && breakDuration === 0) || (!isBreak && count === 0);
